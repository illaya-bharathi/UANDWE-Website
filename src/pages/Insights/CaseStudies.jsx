      ...
      <div className="relative z-10 w-full px-[4%] pt-[120px] pb-24">
        ...
        >
          <motion.div variants={itemVariants}>
            <SectionHeading
              titlePart1={t("insights.caseStudies.titlePart1", "Client")}
              titlePart2={t("insights.caseStudies.titlePart2", "Success Stories")}
              className="!mb-6 uppercase"
            />
          </motion.div>
          <motion.div variants={itemVariants}>
            <Paragraph
              text={t("insights.caseStudies.description", "Discover how we've partnered with global leaders to engineer breakthrough silicon, hardware, and embedded solutions.")}
              animated={false}
            />
          </motion.div>
        </motion.div>
        ...
        >
          {CASE_STUDIES.map((study) => {
            const title = t(`insights_content.caseStudyData.${study.id}.title`, study.title);
            const client = t(`insights_content.caseStudyData.${study.id}.client`, study.client);
            const category = t(`insights_content.caseStudyData.${study.id}.category`, study.category);
            const impact = t(`insights_content.caseStudyData.${study.id}.impact`, study.impact);
            const overview = t(`insights_content.caseStudyData.${study.id}.overview`, study.overview);
            const author = t(`insights_content.caseStudyData.${study.id}.author`, study.author);
            const role = t(`insights_content.caseStudyData.${study.id}.role`, study.role);
            ...
            return (
              <motion.div
                ...
                onClick={() => {
                  navigate(`/resources/casestudies/${study.id}`)
                }}
              ...
              >
                ...
                <div className="flex-1 p-5 sm:p-7 md:p-8 lg:p-10 flex flex-col justify-center">
                  {/* Client & Impact */}
                  <div className="flex flex-wrap items-center gap-4 text-emerald-400 text-xs sm:text-sm font-semibold mb-4 uppercase tracking-wider">
                    <span className="flex items-center gap-1.5"><Building size={14} /> {client}</span>
                    <span className="w-1 h-1 rounded-full bg-white/30" />
                    <span className="flex items-center gap-1.5"><TrendingUp size={14} /> {impact}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-auto pt-5 sm:pt-6 border-t border-white/5">
                    ...
                    <div className="flex items-center gap-2 text-white/40 text-[10px] sm:text-xs font-semibold tracking-wider uppercase group-hover:text-[#ff6b1a] transition-colors duration-300">
                      <span className="hidden sm:inline">{t("insights.caseStudies.viewCaseStudy", "View Case Study")}</span>
